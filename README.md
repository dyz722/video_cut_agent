...
启动后会进入一个终端欢迎面板，展示当前项目目录、主模型协议/模型、常用命令和使用提示。输入区采用类似 Codex 的 `›` 提示和底部状态栏，状态栏会显示当前模型与项目目录。模型请求、工具执行、后台等待时会显示动态状态，例如 `thinking with ...`、`running tool: ...`，避免长任务看起来像卡死。默认展示 agent 的可观察进展事件和工具调用摘要，大段 `bash` 输出、skill 全文、JSON 等会折叠到 `/logs` 中；运行中可用 `/status`、`/live`、`/logs` 查看进度和日志，需要排查时可用 `/logs full`、`/verbose on` 或 `/logview` 展开。事件和工具日志会持久化到当前项目的 `.veoai/logs/`。`veoai` 会保持在交互会话中；如果主模型网关返回 502、上游禁止访问、模型无权限等错误，会在终端给出诊断提示而不是直接崩溃退出。
...
- `/resume`：查看当前工作目录下的历史会话，输入编号或 ID 后直接恢复。
...
- `/logs clear`：清空工具日志。
- `/logview`：打开本地 Web 日志查看器，查看持久化事件和工具调用。
- `/live`：查看最近 agent 运行事件，例如计划摘要、工具调用、结果观察、错误和后台通知。
- `/status`：查看当前运行状态、当前步骤和是否已请求停止。
- `/stop`：请求停止当前运行中的 agent；正在进行的模型或工具调用会先返回。
...
<当前目录或指定项目目录>/
  ...
  output/         渲染成片
  .veoai/         当前项目的会话上下文、元数据与 logs/*.jsonl
```

会话保存在当前项目目录的 `.veoai/sessions/` 下。你在同一个工作目录重新进入 `veoai` 后，可以用 `/resume` 选择继续之前的会话；恢复后终端会回放该会话中用户与 agent 的历史对话，方便接着上次上下文继续。不同工作目录、不同会话的上下文不会互相混用。
